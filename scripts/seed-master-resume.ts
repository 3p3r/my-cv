...
async function waitForBackend(): Promise<void> {
  ...
  throw new Error(`Backend did not become healthy within ${HEALTH_TIMEOUT_MS / 1000}s (${BACKEND_HEALTH_URL})`)
}
...
async function uploadMasterResume(): Promise<void> {
  ...
  console.log(`Master resume seeded: ${result.resume_id ?? 'unknown'} (${result.processing_status ?? 'unknown'})`)
  if (result.processing_status === 'failed') {
    console.warn('LLM parsing failed — configure resume-matcher/apps/backend/.env with an API key for tailoring.')
  }
}
