...
async function testLocalModels(): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Models endpoint failed (${response.status}): ${MODELS_URL}`,
    );
  }
  ...
  if (!ids.includes(LOCAL_LLM_MODEL)) {
    throw new Error(
      `Configured model "${LOCAL_LLM_MODEL}" not in ${ids.join(", ")}`,
    );
  }
}
...
async function testBackendHealth(): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Backend health failed (${response.status}): ${BACKEND_HEALTH_URL}`,
    );
  }
...
async function testChatCompletion(): Promise<void> {
  ...
  console.log(
    `Chat completion via ${LOCAL_LLM_PROVIDER}/${LOCAL_LLM_MODEL}: ${text.slice(0, 120)}...`,
  );
}
...
if (isMain) {
  console.log(
    `Default LLM: ${LOCAL_LLM_PROVIDER} ${LOCAL_LLM_MODEL} @ ${LOCAL_LLM_API_BASE}`,
  );
  ...
  try {
    ...
    console.log(
      "Backend not running — skipping health check (start with npm run backend).",
    );
  }
